...
import {
    ...
    inNetwork_DeleteById,
    inNetwork_GetByBuildingId,
    inNetwork_InsertByData
} from "../../api/Api";
...
export const ImportInnetwork = ({location_id, building_id, reload, pricingInNetwork = []}) => {
    ...
    const onClick = () => {
        setProgress(-1);
        inNetwork_GetByBuildingId(selected_building_id).then(copyItems => {
            return PromiseAllWithProgress(
                pricingInNetwork.map(p => inNetwork_DeleteById(p.id)),
                v => setProgress(v ? -v : -1)
            ).then(
